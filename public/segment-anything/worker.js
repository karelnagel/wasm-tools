...
async function fetchArrayBuffer(url, cacheModel = true) {
  if (!cacheModel) return new Uint8Array(await (await fetch(url)).arrayBuffer());
...
class SAMModel {
  ...
  static async getInstance(modelURL, modelID) {
    if (!this.instance[modelID]) {
      ...
      this.instance[modelID] = new Model(weightsArrayU8, /tiny|mobile/.test(modelID));
    } else {
  ...
  static setImageEmbeddings(imageArrayU8) {
    ...
    if (this.imageArrayHash[this.currentModelID] === imageArrayHash && this.instance[this.currentModelID]) {
      self.postMessage({
        status: "embedding",
